...
const Line = ({name, score, onChangeLanguage }) => {
  ...
  return (
    ...
    <div style={{display: 'flex', alignItems: 'center', flex: 1, justifyContent: 'space-between', margin: '0 20px'}}>
    {score < 20 ? '🥀' : '🌹'}
    ...
    <button
    onClick={onChangeLanguage} style={{fontSize: 28}}>💦</button>
    </div>
  )
}
...
function App() {
  ...
  const onChangeLanguage = (language) => {
    ...
    const newScore = updatedItems[index].score + (updatedItems[index].score < 20 ? 1 : 0);
    ...
    if (rnJesus !== 4) {
      ...
      setLog(oldLogs => [`${updatedItems[index].name} n'a pas pris 1 🟰.`, ...oldLogs])
    }
  ...
  return (
    ...
    {languages.map((language) => 
      <Line key={language.name} name={language.name} score={language.score} onChangeLanguage={() => onChangeLanguage(language)}  />
    )}
    </div>
    <div style={{textAlign: 'left', margin: '1rem 0', backgroundColor: '#121212', borderRadius: 8, padding: 12}}>
    <div style={{display: 'flex', justifyContent: 'space-between'}}>
    <div>Taux de réussite total : </div><div>80%</div>
    </div>
    <div style={{display: 'flex', justifyContent: 'space-between'}}>
    <div>Taux de réussite partiel : </div><div>10%</div>
    </div>
    <div style={{display: 'flex', justifyContent: 'space-between'}}>
    <div>Taux d&apos;échec : </div><div>10%</div>
    </div>
    </div>
    </div>
    ...
    <div style={{textAlign: 'left', height: 400, width: WIDTH, overflowY: 'scroll'}}> 
    {log.map((l, i) => <div style={{fontSize: '1.6rem'}} key={i}>{l}</div>)}
    </div>
    </div>
    </div>
    </header>
